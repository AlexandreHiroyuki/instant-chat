...
io.on('connection', socket => {
	...
	socket.on('create', user => {
		if (user.nickname) {
			...
			console.log('[on create]', nickname, rooms[roomCode].users, socketRoomCode)
		}
	})
	...
	socket.on('join', (roomCode, user) => {
		...
		if (rooms[roomCode].users.has(user.nickname)) {
			...
			socket.join(roomCode)
			socket.emit('joined', [...rooms[roomCode].users.values()], rooms[roomCode].messages)
		...
		console.log('[on join]', nickname, [...rooms[roomCode].users.values()], socketRoomCode)
	})
